...
int main(int argc, char*argv[]){
	...
	float total_sw;
        ...
        std::string logfilename_PS_AND_PL1 = "power_monitoring_" + (std::string)argv[1] + "_" + "_SW.txt";
        std::string logfilename_PS_AND_PL2 = "power_monitoring_" + (std::string)argv[1] + "_" + "_HW.txt";
...
//	float   *final_data     = (float*)sds_alloc(data_matrix_size_bytes);
	...
	//HARDWARE LEARN FUNCTION------------------------
        power_monitoring_start(logfilename_PS_AND_PL2,2);
#ifdef __SDSOC__
        total_accLearn_StartStamp = sds_clock_counter();
#endif
        high_resolution_clock::time_point thw1 = high_resolution_clock::now();
	#pragma SDS resource(1)
	#pragma SDS async(1);
	toplevelHW_learn(data, similarity, numUsers, numItems, 0, 121);

	#pragma SDS resource(2)
    	#pragma SDS async(2);
	toplevelHW_learn(data, similarity, numUsers, numItems, 121, 242);

	#pragma SDS resource(3)
    	#pragma SDS async(3);
        toplevelHW_learn(data, similarity, numUsers, numItems, 242, 363);

    	#pragma SDS resource(4)
    	#pragma SDS async(4);
        toplevelHW_learn(data, similarity, numUsers, numItems, 363, 524);

        #pragma SDS resource(5)
        #pragma SDS async(5);
        toplevelHW_learn(data, similarity, numUsers, numItems, 524, 725);

        #pragma SDS resource(6)
        #pragma SDS async(6);
        toplevelHW_learn(data, similarity, numUsers, numItems, 725, 926);

        #pragma SDS resource(7)
        #pragma SDS async(7);
        toplevelHW_learn(data, similarity, numUsers, numItems, 926, 1287);

        #pragma SDS resource(8)
        #pragma SDS async(8);
        toplevelHW_learn(data, similarity, numUsers, numItems, 1287, numUsers);
	...
	#pragma SDS wait(1)
        ...
        #pragma SDS wait(7)
        #pragma SDS wait(8)
        high_resolution_clock::time_point thw2 = high_resolution_clock::now();
	...
	//SOFTWARE LEARN FUNCTION------------------------
        power_monitoring_start(logfilename_PS_AND_PL1,2);
	total_SoftwareLearn_StartStamp = sds_clock_counter();
        high_resolution_clock::time_point tsw1 = high_resolution_clock::now();
	learn_software(data, similarity_software, numUsers, numItems);
        high_resolution_clock::time_point tsw2 = high_resolution_clock::now();
...
//  printf("total time accelerator Predict = %lld\n",total_no_accPredict);
	...
	printf("-----------------\n");
        ...
        duration<double> time_span_hw = duration_cast<duration<double>>(thw2 - thw1);
        duration<double> time_span_sw = duration_cast<duration<double>>(tsw2 - tsw1);
        std::cout << "hardware took me " << time_span_hw.count() << " seconds.";
        std::cout << "\nsoftware took me " << time_span_sw.count() << " seconds.";
        printf("\n-----------------\n");
        std::cout << std::endl;
