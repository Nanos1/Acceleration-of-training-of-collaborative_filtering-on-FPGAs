...
void pickUser(float* data, int _user0[ITEM_SIZE], int i, int k,int numItems, int &N0){
...
void pickNextUser(float* data, stream_data_fifo &_data, int j, int k, int numItems, int &Ni){
 
//       #pragma HLS INLINE
        float tmpdt;
        pickNextUser: for(int h=0;h<ITEM_SIZE;h++){
        #pragma HLS PIPELINE
                tmpdt =  data[j*numItems+k*ITEM_SIZE+h];
                _data << tmpdt;
                Ni += (tmpdt!=0) ? 1 : 0;
        }
}
...
void toplevelHW_learn(float* data, float* similarity, int numUsers, int numItems, int indx, int lmtindx){
	...
	int _user0[ITEM_SIZE], _user1[ITEM_SIZE], _user2[ITEM_SIZE], _user3[ITEM_SIZE], _user4[ITEM_SIZE], _user5[ITEM_SIZE], _user6[ITEM_SIZE], _user7[ITEM_SIZE], _user8[ITEM_SIZE], _user9[ITEM_SIZE], _user10[ITEM_SIZE], _user11[ITEM_SIZE], _user12[ITEM_SIZE], _user13[ITEM_SIZE], _user14[ITEM_SIZE], _user15[ITEM_SIZE], _user16[ITEM_SIZE], _user17[ITEM_SIZE], _user18[ITEM_SIZE], _user19[ITEM_SIZE], _user20[ITEM_SIZE], _user21[ITEM_SIZE], _user22[ITEM_SIZE], _user23[ITEM_SIZE], _user24[ITEM_SIZE], _user25[ITEM_SIZE], _user26[ITEM_SIZE], _user27[ITEM_SIZE], _user28[ITEM_SIZE], _user29[ITEM_SIZE], _user30[ITEM_SIZE], _user31[ITEM_SIZE], _user32[ITEM_SIZE], _user33[ITEM_SIZE], _user34[ITEM_SIZE], _user35[ITEM_SIZE], _user36[ITEM_SIZE], _user37[ITEM_SIZE], _user38[ITEM_SIZE], _user39[ITEM_SIZE];
        ...
        int N0=0, N1=0, N2=0, N3=0, N4=0, N5=0, N6=0, N7=0, N8=0, N9=0, N10=0, N11=0, N12=0, N13=0, N14=0, N15=0, N16=0, N17=0, N18=0, N19=0, N20=0, N21=0, N22=0, N23=0, N24=0, N25=0, N26=0, N27=0, N28=0, N29=0, N30=0, N31=0, N32=0, N33=0, N34=0, N35=0, N36=0, N37=0, N38=0, N39=0;
        int Ni=0;
	int Item_tiles = numItems/ITEM_SIZE, jjj0, isfull0=0, flg = Item_tiles-1, ii=indx;
		...
		for(int k=0;k<Item_tiles;k++){
			...
			pickUser(data, _user0, i, k, numItems, N0);
 			pickUser(data, _user1, i+1, k, numItems, N1);
 			pickUser(data, _user2, i+2, k, numItems, N2);
 			pickUser(data, _user3, i+3, k, numItems, N3);
                        pickUser(data, _user4, i+4, k, numItems, N4);
                        pickUser(data, _user5, i+5, k, numItems, N5);
                        pickUser(data, _user6, i+6, k, numItems, N6);
                        pickUser(data, _user7, i+7, k, numItems, N7);
                        pickUser(data, _user8, i+8, k, numItems, N8);
                        pickUser(data, _user9, i+9, k, numItems, N9);
                        pickUser(data, _user10, i+10, k, numItems, N10);
                        pickUser(data, _user11, i+11, k, numItems, N11);
                        pickUser(data, _user12, i+12, k, numItems, N12);
                        pickUser(data, _user13, i+13, k, numItems, N13);
                        pickUser(data, _user14, i+14, k, numItems, N14);
                        pickUser(data, _user15, i+15, k, numItems, N15);
                        pickUser(data, _user16, i+16, k, numItems, N16);
                        pickUser(data, _user17, i+17, k, numItems, N17);
                        pickUser(data, _user18, i+18, k, numItems, N18);
                        pickUser(data, _user19, i+19, k, numItems, N19);
                        pickUser(data, _user20, i+20, k, numItems, N20);
                        pickUser(data, _user21, i+21, k, numItems, N21);
                        pickUser(data, _user22, i+22, k, numItems, N22);
                        pickUser(data, _user23, i+23, k, numItems, N23);
                        pickUser(data, _user24, i+24, k, numItems, N24);
                        pickUser(data, _user25, i+25, k, numItems, N25);
                        pickUser(data, _user26, i+26, k, numItems, N26);
                        pickUser(data, _user27, i+27, k, numItems, N27);
                        pickUser(data, _user28, i+28, k, numItems, N28);
                        pickUser(data, _user29, i+29, k, numItems, N29);
                        pickUser(data, _user30, i+30, k, numItems, N30);
                        pickUser(data, _user31, i+31, k, numItems, N31);
                        pickUser(data, _user32, i+32, k, numItems, N32);
                        pickUser(data, _user33, i+33, k, numItems, N33);
                        pickUser(data, _user34, i+34, k, numItems, N34);
                        pickUser(data, _user35, i+35, k, numItems, N35);
                        pickUser(data, _user36, i+36, k, numItems, N36);
                        pickUser(data, _user37, i+37, k, numItems, N37);
                        pickUser(data, _user38, i+38, k, numItems, N38);
                        pickUser(data, _user39, i+39, k, numItems, N39);			
			for(int j=i+1;j<numUsers;j++){
				...
				pickNextUser(data, _data, j, k, numItems, Ni);
				compute_sim(_user0, _user1, _user2, _user3, _user4, _user5, _user6, _user7, _user8, _user9, _user10, _user11, _user12, _user13, _user14, _user15, _user16, _user17, _user18, _user19, _user20, _user21, _user22, _user23, _user24, _user25, _user26, _user27, _user28, _user29, _user30, _user31, _user32, _user33, _user34, _user35, _user36, _user37, _user38, _user39 , _data, _sum0, _sum1, _sum2, _sum3, _sum4, _sum5, _sum6, _sum7, _sum8, _sum9, _sum10, _sum11, _sum12, _sum13, _sum14, _sum15, _sum16, _sum17, _sum18, _sum19, _sum20, _sum21, _sum22, _sum23, _sum24, _sum25, _sum26, _sum27, _sum28, _sum29, _sum30, _sum31, _sum32, _sum33, _sum34, _sum35, _sum36, _sum37, _sum38, _sum39, N0, N1, N2, N3, N4, N5, N6, N7, N8, N9, N10, N11, N12, N13, N14, N15, N16, N17, N18, N19, N20, N21, N22, N23, N24, N25, N26, N27, N28, N29, N30, N31, N32, N33, N34, N35, N36, N37, N38, N39, Ni);
	...
	for(int i=ii;i<lmtindx;i++){
			...
			N0=0;
				...
				pickNextUser(data, _data, j, k, numItems, Ni);
				compute_Remaining_sim(_user0, _data, _sum0, N0, Ni);
...
void simisfull(float* similarity, int _sim[USER_SIZE], int i, int jjj, int numUsers, int k, int flg){
	...
	float temp_setUV;
		...
		ttemp_sim = temp_sim;
		if(k==flg){
			temp_setUV = temp_sim/10000; // setU \/ setV
			temp_sim = temp_sim%10000; // setU /\ setV 
			if(temp_sim>1){
				ttemp_sim = temp_sim/(-temp_sim+temp_setUV);
			}
			else{
				ttemp_sim = 0;
			}
...
void compute_Remaining_sim(int _user0[ITEM_SIZE], stream_data_fifo &_data, int &_sum0, int N0, int Ni){
        int temp_user0;
        int temp_data;
...
//        #pragma HLS UNROLL factor=2
		...
		_data >> temp_data;
/*                _sum0 += (temp_user0!=0) ? (temp_data!=0) ? 1 : 0 : 0; 
		_sum1 += (temp_user1!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum2 += (temp_user2!=0) ? (temp_data!=0) ? 1 : 0 : 0;
		_sum3 += (temp_user3!=0) ? (temp_data!=0) ? 1 : 0 : 0;  
                _sum4 += (temp_user4!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum5 += (temp_user5!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum6 += (temp_user6!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum7 += (temp_user7!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum8 += (temp_user8!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum9 += (temp_user9!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum10 += (temp_user10!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum11 += (temp_user11!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum12 += (temp_user12!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum13 += (temp_user13!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum14 += (temp_user14!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum15 += (temp_user15!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum16 += (temp_user16!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum17 += (temp_user17!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum18 += (temp_user18!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum19 += (temp_user19!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum20 += (temp_user20!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum21 += (temp_user21!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum22 += (temp_user22!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum23 += (temp_user23!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum24 += (temp_user24!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum25 += (temp_user25!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum26 += (temp_user26!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum27 += (temp_user27!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum28 += (temp_user28!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum29 += (temp_user29!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum30 += (temp_user30!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum31 += (temp_user31!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum32 += (temp_user32!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum33 += (temp_user33!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum34 += (temp_user34!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum35 += (temp_user35!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum36 += (temp_user36!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum37 += (temp_user37!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum38 += (temp_user38!=0) ? (temp_data!=0) ? 1 : 0 : 0;
                _sum39 += (temp_user39!=0) ? (temp_data!=0) ? 1 : 0 : 0;
*/	
