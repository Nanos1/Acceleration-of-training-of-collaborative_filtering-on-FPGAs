...
void learn_software(float* data, float* similarity_software, int numUsers, int numItems){
	int ii=0;
	float set_of_U[40], set_of_UV[40], set_of_V=0;
	...
	float setU, setV, setUV;
        for(int i=ii;i<numUsers;i++){
                for(int j=i+1;j<numUsers;j++){
                        ...
                        if (setUV>1){
                                similarity_software[i*numUsers+j]=(setUV)/(setU+setV-setUV);
                        }
                        else{
...
void calc_sim(float* similarity_software, float set_of_U[40], float set_of_UV[40], float set_of_V, int i, int j, int numUsers){
	int ii=i%40;
	if (set_of_UV[ii]>1){
        	similarity_software[i*numUsers+j]=(set_of_UV[ii])/(set_of_U[ii]+set_of_V-set_of_UV[ii]);
        }
        else{
        	similarity_software[i*numUsers+j]=0;
        }
