...
//	float   *final_data     = (float*)sds_alloc(data_matrix_size_bytes);
	float     *U              = (float*)sds_alloc(sim_matrix_size_bytes);
	float     *V              = (float*)sds_alloc(sim_matrix_size_bytes);
	int     *setN           = (int*)sds_alloc(sim_matrix_size_bytes);
	...
	initialize_data(data, similarity, U, V, setN, numUsers, numItems);
	...
	//HARDWARE LEARN FUNCTION------------------------
        ...
        #pragma SDS async(1);
        toplevelHW_learn(data, similarity, setN , U, V, numUsers, numItems, 0, 201);

        #pragma SDS resource(2)
        #pragma SDS async(2);
        toplevelHW_learn(data, similarity, setN , U, V, numUsers, numItems, 201, 402);

        #pragma SDS resource(3)
        #pragma SDS async(3);
        toplevelHW_learn(data, similarity, setN , U, V, numUsers, numItems, 402, 803);

        #pragma SDS resource(4)
        #pragma SDS async(4);
        toplevelHW_learn(data, similarity, setN , U, V, numUsers, numItems, 803, numUsers);
